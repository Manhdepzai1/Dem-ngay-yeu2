// ===== THÊM DÒNG NÀY LÊN TRÊN CÙNG (FIX LỖI MODAL) =====
const photoUrls = [
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // ===== GIỮ NGUYÊN, CHỈ XÓA const =====
  // const photoUrls = [ ... ];   <-- XÓA DÒNG NÀY
  ...
  function createHeartPhotoCentered(idx, total) {
    ...
    photo.dataset.index = idx % photoUrls.length;
    ...
    const scale = (window.innerWidth <= 480) ? 11 : 17;
    ...
    setTimeout(() => {
      ...
      photo.style.transform =
        `translate(-50%, -50%) translate(${targetX - centerX}px, ${targetY - centerY}px)`;
...
document.addEventListener("click", function(e) {
  ...
  if (e.target.classList.contains("photo")) {
    ...
    currentIndex = parseInt(e.target.dataset.index) || 0;
